    ...
    if (response.ok) {
        ...
        if (json.success) {
            ...
            cartList.forEach(item => {
                ...
                let total = (item.product.price - ((item.product.price / 100) * item.product.discount));
                ...
                cart_item_clone.querySelector("#fs-cart-item-price").innerHTML = new Intl.NumberFormat("en-US", {minimumFractionDigits: 2}).format(total);
                cart_item_clone.querySelector("#fs-cart-item-total-price").innerHTML = new Intl.NumberFormat("en-US", {minimumFractionDigits: 2}).format(total * item.qty);
                cart_container.appendChild(cart_item_clone);

                sub_total += (total * item.qty);
                total_discount += ((item.product.price - total) * item.qty);
